...
# Text tool
text_active = False    # True = typing
text_pos = (0, 0)      
text_buffer = ""       # characters typed 
...
def flood_fill(surface, pos, fill_color):
    x, y = pos
    target = surface.get_at((x, y))[:3]   # color replacing
...
def draw_toolbar():
    ...
    screen.blit(hints, (10, 80))   # second hint row (partially off is fine)
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            # While text tool is active, route keys to the text buffer
            if text_active:
                if event.key == pygame.K_RETURN:
                    # Commit text permanently onto canvas
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            # Left click = start drawing (only below toolbar)
