...
# coin types(value, color, size, spawn weight)
COIN_TYPES = [
    (1, YELLOW,          20, 60),
    (3, (192,192,192),   22, 30),
    (5, BLUE,            25, 10),
...
class Player(pygame.sprite.Sprite):
    ...
    def move(self):
        ...
        # turn off nitro when time runs out
...
class Obstacle(pygame.sprite.Sprite):
    TYPES = [
        ((50, 20), (30, 30, 30)),    # oil spill
        ((30, 30), (60, 40, 10)),    # pothole
    ]
    def __init__(self):
        super().__init__()
...
class Barrier(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.dx = random.choice([-2, 2])   # horizontal direction
...
class PowerUp(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect = self.image.get_rect()
        self.rect.center = (random.randint(40, W - 40), -60)
        self.born = time.time()   # track when spawned

    def update(self):
        self.rect.y += 2
        # disappear if not collected within 12 seconds
...
def run_game(screen, settings, sounds, player_name):
    ...
    active_pu     = None   # current powerup name
    active_pu_end = 0      # when it expires
    ...
    pygame.time.set_timer(COIN_EV,    2000)   # coin every 4s
    pygame.time.set_timer(OBS_EV,     2500)   # obstacle every 3.5s
    pygame.time.set_timer(BARRIER_EV, 6000)   # barrier every 6s
    pygame.time.set_timer(POWER_EV,   7000)   # powerup every 7s

    while True:
        clock.tick(30)
        distance += 1   # 30 units = 1 meter
        ...
        # barrier collision — same as enemy
        ...
        # draw shield outline around car
        if player.shield:
