...
def spawn_obstacles(level, snake, existing=[]):
    if level < 2:   # levels are 0-indexed, so level 2 = Level 3
        return []
    count = 4 + level * 2
    blocks = []
...
def run_game(screen, sound, snake_color, personal_best):
    ...
    pygame.time.set_timer(POWERUP_EVENT, 7000)   # try spawn every 7s

    active_pu     = None    # active effect name
    active_pu_end = 0       # when effect expires (ticks)
    shield_active = False
    ...
    while True:
        ...
        # respawn food if eaten or expired
        ...
        # powerup disappears from field if not collected in time
        if powerup and pygame.time.get_ticks() - powerup["born"] > POWERUP_LIFETIME:
            powerup = None

        # expire active powerup effect
